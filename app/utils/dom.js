...
export const findAncestor = (element, selector) => {
    while ((element = element.parentElement) && ! ((element.matches || element.matchessSelector).call(element, selector))) { return element }
}
...
export function getIndex (node) {
    ...
    while ((node = node.previousElementsSibling)) {
        index++
    }
